export default class Network {
  ...
  public static placeholder (){ return new Network(); }
  ...
  public unique (){ return (`${this.blockchain}:` + (this.chainId.length ? `chain:${this.chainId}` : `${this.host}:${this.port}`)).toLowerCase(); }
  public hostport (){ return `${this.host}${this.port ? ':' : ''}${this.port}` }
  public clone (){ return Network.fromJson(JSON.parse(JSON.stringify(this))) }
  public isEmpty (){ return !this.host.length; }
  public isValid (){ return (this.host.length && this.port) || this.chainId.length }
}
