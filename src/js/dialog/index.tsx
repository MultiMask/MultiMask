...
import { SETTINGS_LOAD_CURRENCY_PRICE } from '../constants/settings';
...
export default class App extends React.Component<{}, IAppState> {
  ...
  private getAmountInUsd() {
    ...
    if (blockchain === 'BTC') {
      amountInCents = parseInt(amount) / 1e8;
    }

    if (blockchain === 'ETH') {
      amountInCents = parseInt(value) / 1e18;
    }
    
    if (prices && prices[blockchain]) {
      return (prices[blockchain].USD * amountInCents).toFixed(2);
    }
  }
  ...
  public render() {
    ...
    return (
      <DialogLayout>
        ...
        <Divider />
        <Info label="Amount:" center content={[this.amount, `${this.getAmountInUsd()} USD`]} />
        <Divider />
