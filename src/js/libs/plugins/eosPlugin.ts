...
export default sender => {
  ...
  return {
    MAINNET: ntx.EOS.network[0].chainId,
    JUNGLE: ntx.EOS.network[1].chainId,
        ...
        get(eosInstance, method) {
          ...
          return (...args) => {
            if (args.find(arg => arg.hasOwnProperty('keyProvider'))) {
              throw new Error('Already use keyProvider');
            }
            ...
            if (!requiredFields.isValid()) {
              throw new Error('Required fields is invalid');
            }
            ...
            return new Promise((resolve, reject) => {
              _eos(Object.assign(_options, { httpEndpoint, signProvider }))
                [method](...args)
                .then(result => {
...
const requestParser = async (_eos, signargs, httpEndpoint, possibleSigner, chainId) => {
  ...
  const contracts = signargs.transaction.actions.map(action => action.account).reduce((acc, contract) => {
  ...
  await Promise.all(
    contracts.map(async contractAccount => {
  ...
  return Promise.all(
    signargs.transaction.actions.map(async (action, index) => {
